 ...
 * by the annotation {@link AutoRpcServiceImpl} on the implementation of the service.  Such
 * implementation beans must also have the {@link RpcService} annotation associated with them; either
 * on the implementation class itself or, preferably, on an interface that the implementation implements.
 * </p>
 * <p>
 * <p>The path for exposing the service is obtained from {@link RpcService#value()}, but it is also
 * possible to define additional paths on {@link AutoRpcServiceImpl#additionalPaths()}.</p>
 * <p>
 ...
 * </pre>
 * <p>
 * <p>Note that this class replaces {@link AutoRpcServiceExporter}.  See that class' javadoc
 * for details.</p>
 */
...
public class AutoRpcServiceImplExporter implements BeanFactoryPostProcessor {
    ...
    private static Map<String, String> findServiceBeanDefinitions(ConfigurableListableBeanFactory beanFactory) {
        ...
        for (String beanName : beanFactory.getBeanDefinitionNames()) {
            AutoRpcServiceImpl autoJsonRpcServiceImplAnnotation = beanFactory.findAnnotationOnBean(beanName, AutoRpcServiceImpl.class);
            RpcService jsonRpcServiceAnnotation = beanFactory.findAnnotationOnBean(beanName, RpcService.class);
            ...
            if (null != autoJsonRpcServiceImplAnnotation) {
                ...
                if (null == jsonRpcServiceAnnotation) {
                    throw new IllegalStateException("on the bean [" + beanName + "], @" +
                            AutoRpcServiceImpl.class.getSimpleName() + " was found, but not @" +
                            RpcService.class.getSimpleName() + " -- both are required");
                }
    ...
    private void registerServiceProxy(DefaultListableBeanFactory defaultListableBeanFactory, String servicePath, String serviceBeanName) {
        BeanDefinitionBuilder builder = BeanDefinitionBuilder.rootBeanDefinition(ServiceExporter.class).addPropertyReference("service", serviceBeanName);
